...
// función de login del usuario y lo emvia a req.body
function login(req,res){
    var data = req.body;

    User.findOne({email: data.email},(err,user_data)=>{
        if(err){
            res.status(500).send({message: 'Error en el servidor'}); // < devuelve un error si no hay conexion con mongo
        }else{
            if(user_data){
...
// Lista los usuarios almacenados en la colección

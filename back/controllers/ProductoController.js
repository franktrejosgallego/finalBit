...
function registrar(req,res){
    ...
    if(req.files){
        ...
        producto.save((err,producto_save)=>{ // Función flecha para grabar productos
            if(err){
        ...
        producto.save((err,producto_save)=>{
            if(err){
                res.status(500).send({message: 'Error en el servidor'}); // envia un mensaje si no tiene conexion con la bd
            }else{
                if(producto_save){
                    res.status(200).send({produto: producto_save});
                }else{
...
function listar(req,res){ // Aca lista por parámetros
...
// Editar los productos y guarda las imagenes en /uploads/productos
function editar(req,res){
    ...
    if(req.files.imagen){
        ...
        // Busca el path local con la imagen del producto
        var imagen_path = req.files.imagen.path;
        var name = imagen_path.split('\\');
        var imagen_name = name[2];
...
// Obtienen el producto dentro de la colección en mongo
...
// retorna al path local si no encontró una imagen
...
module.exports ={  // < Exporta los modules del crud para la vista de productos
    registrar,
    listar,
    editar,
    obtener_producto,
